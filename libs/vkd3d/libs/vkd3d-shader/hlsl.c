        ...
        struct vkd3d_shader_code *out, struct vkd3d_shader_message_context *message_context)
{
    ...
    size_t sz = min(sizeof(sourcecopy) - 1, hlsl->size);
    strncpy(sourcecopy, hlsl->code, sz);
    sourcecopy[sz] = 0;
    ...
    if (hlsl_source_info->entry_point) {
        WARN("hlsl_source_info->entry_point=%s\n", hlsl_source_info->entry_point);
    } else {
        WARN("!hlsl_source_info->entry_point\n");
    }

    entry_point = hlsl_source_info->entry_point ? hlsl_source_info->entry_point : "maine";
    ...
    if (!entry_func)
    {
        ...
        hlsl_error(&ctx, &loc, VKD3D_SHADER_ERROR_HLSL_NOT_DEFINED,
                "Entry point \"%s\" is not defined in: %s /%d", entry_point, sourcecopy, hlsl->size);
        hlsl_ctx_cleanup(&ctx);
